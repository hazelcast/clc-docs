...
* Added the `\exit` and `\echo` commands. These commands are available in the script and interactive modes. See: [echo](https://docs.hazelcast.com/clc/latest/echo) and [exit](https://docs.hazelcast.com/clc/latest/exit) command documentation.
* Added `--wait` flag to `viridian` commands.
* Added the ability to change the interactive mode prompt and banner. See: [def](https://docs.hazelcast.com/clc/latest/def).
...
* Viridian commands display an API key selector if the there exists more than one API keys.
* Viridian cluster list is paged.

== Changes

* Project creator uses the `defaults` field in `template.yaml` instead of the `defaults.yaml`.
* Templte name is optional with `clc project create` command. A selector is displayed if the template name is not specified.
